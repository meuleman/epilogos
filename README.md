...
To compute epilogos, you must have the external programs `bgzip` and `starch` (the latter is part of [BEDOPS](https://github.com/bedops/bedops)) in your PATH environment variable. 
...
<aside class="notice">
See [below](#single-chromosome-execution) for important differences between `computeEpilogos.sh` and `computeEpilogos_singleChromosomeSingleProcessor.sh` scripts
</aside>
...
<aside class="notice">
Use `gunzip` to decompress the input data file `chr1_127epigenomes_15observedStates.txt.gz` before using it.
</aside>
