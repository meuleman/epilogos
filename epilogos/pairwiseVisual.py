...
def createTopScoresTxt(filePath, locationArr, chrDict, distanceArr, maxDiffArr, nameArr, pvals, onlySignificant, mhPvals, width):
    ...
    if not onlySignificant:
        ...
        indices = np.intersect1d(indices, meanAndMax(distanceArr, 100, width), assume_unique=True)
...
def meanAndMax(scoreArr, k, w):
    ...
    while k > 0:
        try:
            ...
            for i in findBestIndex(scoreArr, i, w):
...
def findBestIndex(scoreArr, i, width):
    farthestLeft = i - width
...
def greatestHitsNoSignificance(filePath, locationArr, chrDict, distanceArr, maxDiffArr, nameArr, zScores, width):
    ...
    indices = meanAndMax(distanceArr, 100, width)
