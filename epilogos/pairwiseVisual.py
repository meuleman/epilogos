...
def createTopScoresTxt(filePath, locationArr, chrDict, distanceArr, maxDiffArr, nameArr, pvals, onlySignificant, mhPvals, width):
    ...
    if onlySignificant:
        ...
        indices, _, comm2 = np.intersect1d(np.where(mhPvals <= 0.1)[0], indices, assume_unique=True, return_indices=True)
        maxIndices = maxIndices[comm2]
...
def findBestIndex(scoreArr, i, width):
    ...
    for j in range(farthestLeft, farthestRight + 1):
        means.append(-np.mean(np.abs(scoreArr[j - width:j + width + 1])))
