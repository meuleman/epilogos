...
def s1Multi(file1Path, file2Path, rowList, totalRows, numStates, expFreqPath, numProcesses, realOrNull):
    ...
    # Start the processes
    with closing(Pool(numProcesses, initializer=_init, initargs=(sharedArr1, sharedArr2, totalRows, numStates))) as pool:
        pool.starmap(s1Score, zip(repeat(file1Path), repeat(file2Path), rowList, repeat(expFreqPath), repeat(realOrNull)))
    pool.join()
...
def s2Multi(file1Path, file2Path, rowList, totalRows, numStates, expFreqPath, numProcesses, realOrNull):
    ...
    # Start the processes
    with closing(Pool(numProcesses, initializer=_init, initargs=(sharedArr1, sharedArr2, totalRows, numStates))) as pool:
        pool.starmap(s2Score, zip(repeat(file1Path), repeat(file2Path), rowList, repeat(expFreqPath), repeat(realOrNull)))
    pool.join()
