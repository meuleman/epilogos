...
def main(fileDirectory, numStates, saliency, outputDirectory, modeOfOperation, expFreqDir):
    ...
    if modeOfOperation == "s" or modeOfOperation == "both":
        ...
        jobName = "writePandas_{}".format(fileTag)
        jobOutPath = outputDirPath / (".out/" + jobName + ".out")
        jobErrPath = outputDirPath / (".err/" + jobName + ".err")
        ...
        computeExpectedWritePy = pythonFilesDir / "computeEpilogosWritePandas.py"

        pythonCommand = "python {} {} {} {}".format(computeExpectedWritePy, fileTag, outputDirPath, numStates)

        slurmCommand = "sbatch --dependency=afterok:{} --job-name={}.job --output={} --error={} --nodes=1 --ntasks=1 --mail-type=END --mail-user=jquon@altius.org --mem-per-cpu=64000 --wrap='{}'".format(jobIDStrWrite, jobName, jobOutPath, jobErrPath, pythonCommand)
