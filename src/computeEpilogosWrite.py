...
def main(file, numStates, outputDirectory, fileTag, verbose):
    ...
    if verbose: print("    Time:", time.time() - tRead, flush=True)
    ...
    if verbose: print("    Time:", time.time() - tCreate, flush=True)

    # Write out the string
    if verbose: print("Writing scores...", flush=True)
    tScore = time.time()
    scoresTxt.write(scoreStr)
    if verbose: print("    Time:", time.time() - tScore, flush=True)
    ...
    print("Total Time:", time.time() - tTotal, flush=True) if verbose else print("    {}\t\t[Done]".format(filename), flush=True)
